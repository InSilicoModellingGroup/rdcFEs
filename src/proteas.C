...
void input (const std::string & file_name, EquationSystems & es)
{
...

  {
    ...
    name = "roiVolCalc/integral"; es.parameters.set<bool>(name) = in(name, false);
...
void save_solution (std::ofstream & csv, EquationSystems & es)
{
  ...
  const bool integralVolCalc = es.parameters.get<bool>("roiVolCalc/integral");
          ...
          if (integralVolCalc)
            {
              ...
              Real temp_tum_volume = 0.0;
              Real temp_nec_volume = 0.0;
              Real temp_oed_volume = 0.0;
              ...
              if (tum_include) tum_volume += temp_tum_volume;
              if (nec_include) nec_volume += temp_nec_volume;
              if (oed_include) oed_volume += temp_oed_volume;
            }
          else
            {
              {
